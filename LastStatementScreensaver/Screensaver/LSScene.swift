...
class LSScene: SKScene, LSManagerDelegate {
    ...
    private func showInformationDisplay() {
        guard let card = getNextCard() else { return }

        let infoDuration = getEstimatedReadingTimeForString(card.answer)

        // Create Intro Name Label Node
        let slidingNameLabel = SKLabelNode(text: card.getName())
        ...
        let nameLabel = SKLabelNode(text: card.getName())
        ...
        let executionDateLabel = SKLabelNode(text: "Executed \(card.getExecutionDateAsString())")
        ...
        let lastStatementLabel = SKLabelNode(text: card.lastStatement)
    ...
    fileprivate func getNextName() -> String {
        guard let randomCard = getNextCard() else {
            return "Error: Card Not Found"
        }
        return randomCard.getName()
    }

    fileprivate func getNextCard() -> Card? {
        ...
        let card = executions[currentExecutionIndex]
