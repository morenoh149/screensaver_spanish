...
final class LSManager {
    ...
    private func getExecutionsJSONString() -> String? {
        // TODO disable the download check in `getExecutionsJSONString()` and just leave the embeddedExecutions grab
        /*
        if executionsJSON != "" {
        ...
        if let embeddedExecutions = loader.getExecutionsStringFromLocal() {
            return embeddedExecutions;
        } else {
            return nil
        }
